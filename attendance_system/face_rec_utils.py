...
def save_training_image(image, user_id, count, save_path):
    ...
    try:
        ...
        print(f"DEBUG: save_training_image called. User: {user_id}, Count: {count}")
        ...
        for (x, y, w, h) in faces:
            ...
            success = cv2.imwrite(file_path, gray[y:y+h, x:x+w])
            print(f"DEBUG: Save success: {success}")
            return True
        print("DEBUG: No faces detected in save_training_image")
        return False
    except Exception as e:
        print(f"DEBUG: Error in save_training_image: {e}")
        return False
